...
delta=[2,3,2.5,1.8,1.6,2];
% delta=2;
h1=[0.7,0.6,0.4,0.5,0.6,0.5];
h2=[0.09,0.07,0.08,0.07,0.07,0.07];
% h=1;
mu1=1;
mu2=0;
...
L=[2.5,2,3,2.8,2.1,2.5];
%initial position
px0=39;
% px=[33.22,27.44,21.66,15.88,10.1,4.32];%0³õÊ¼Îó²î
px=[33,27,21,15,9,3];
...
for j=1:6000
    ...
    e(1,j)=mu1*(px0-px(1)-delta(1)-h1(1)*vx(1)-h2(1)*vx(1)^2-L(1))+mu2*(vx(1)-vx0);
    edot(1)=mu1*(vx0-vx(1)-(h1(1)+2*h2(1)*vx(1))/m(1)*ux(1))+mu2/m(1)*(ux(1)-ax0);
    
    for i=2:N
        e(i,j)=mu1*(px(i-1)-px(i)-delta(i)-h1(i)*vx(i)-h2(i)*vx(i)^2-L(i))+mu2*(vx(i)-vx0);
    ...
    ux(1)=m(i)*(mu1/((h1(1)+2*h2(1)*vx(1))*mu1-mu2)*(vx0-vx(1))+1/(q*C(1)*((h1(1)+2*h2(1)*vx(1))*mu1-mu2))*D(1)...
              +1/(q*C(1)*((h1(1)+2*h2(1)*vx(1))*mu1-mu2))*(k3*tanh(S(1,j))+k4*abs(S(1,j))^eta*sign(S(1,j))));
    for i=2:N
        ux(i)=m(i)*(mu1/((h1(i)+2*h2(i)*vx(i))*mu1-mu2)*(vx(i-1)-vx(i))+1/(q*C(i)*((h1(i)+2*h2(i)*vx(i))*mu1-mu2))*D(i)...
              +1/(q*C(i)*((h1(i)+2*h2(i)*vx(i))*mu1-mu2))*(k3*tanh(S(i,j))+k4*abs(S(i,j))^eta*sign(S(i,j))));
    end
